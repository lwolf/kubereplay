...
type RefineryLister interface {
	...
	List(selector labels.Selector) (ret []*replay.Refinery, err error)
...
// List lists all Refineries in the indexer.
func (s *refineryLister) List(selector labels.Selector) (ret []*replay.Refinery, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*replay.Refinery))
	})
	return ret, err
}
...
type RefineryNamespaceLister interface {
	...
	Get(name string) (*replay.Refinery, error)
...
// List lists all Refineries in the indexer for a given namespace.
func (s refineryNamespaceLister) List(selector labels.Selector) (ret []*replay.Refinery, err error) {
...
// Get retrieves the Refinery from the indexer for a given namespace and name.
func (s refineryNamespaceLister) Get(name string) (*replay.Refinery, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(replay.Resource("refinery"), name)
	}
	return obj.(*replay.Refinery), nil
}
