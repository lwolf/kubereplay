...
type SiloLister interface {
	...
	List(selector labels.Selector) (ret []*replay.Silo, err error)
...
// List lists all Silos in the indexer.
func (s *siloLister) List(selector labels.Selector) (ret []*replay.Silo, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*replay.Silo))
	})
	return ret, err
}
...
type SiloNamespaceLister interface {
	...
	Get(name string) (*replay.Silo, error)
...
// List lists all Silos in the indexer for a given namespace.
func (s siloNamespaceLister) List(selector labels.Selector) (ret []*replay.Silo, err error) {
...
// Get retrieves the Silo from the indexer for a given namespace and name.
func (s siloNamespaceLister) Get(name string) (*replay.Silo, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(replay.Resource("silo"), name)
	}
	return obj.(*replay.Silo), nil
}
